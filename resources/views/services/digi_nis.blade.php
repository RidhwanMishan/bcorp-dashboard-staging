...
@section('title') Services > Digital Services > Natural Intelligence Solutions @endsection
...
<div class="slim-mainpanel company">
    ...
    <div class="slim-pageheader">
        <div>
        @if($userID == 1)
        <div class="level-up"><a href="/services/digital"><button class="btn btn-secondary mg-r-10">Digital Services <i class="fa fa-level-up"></i></button></a>
        </div>
        <a href="/services/digital/bloyalty"><button class="btn btn-primary mg-r-10">BLoyalty</button></a>
        <a href="/services/digital/bloyaltyltd"><button class="btn btn-primary mg-r-10">BLoyalty Ltd</button></a>
        <a href="/services/digital/nis"><button class="btn btn-success mg-r-10">NIS</button></a>
        <a href="/services/digital/nist"><button class="btn btn-primary mg-r-10">NIST</button></a>
        <a href="/services/digital/redtone"><button class="btn btn-primary mg-r-10">REDtone</button></a>
        @else
          @if($redtone)
          <a href="/services/digital/redtone"><button class="btn btn-primary mg-r-10">REDtone</button></a>
          @endif
          ...
          @endif
          @if($bloyaltyltd)
          <a href="/services/digital/bloyaltylte"><button class="btn btn-primary mg-r-10">BLoyalty Ltd</button></a>
          @endif
        @endif
        </div>
        <h6 class="slim-pagetitle">Services - Digital Services - Natural Intelligence Solutions Pte Ltd</h6>
    </div>
        ...
        <div id="PrimaryGPLCard" class="card">
            ...
            <div class="card card-sub-data ">
                <div class="card-body">
                    ...
                    <script>
                        var data = <?php echo json_encode($gpl_breakdown_data); ?>;
                        if (data.length == 0) {
                          data = [100];
                        }
                        var label = <?php echo json_encode($gpl_breakdown_labels); ?>;
                        if (label.length == 0) {
                          label = ['GPL no data'];
                        }
        ...
        <div id="PrimaryPATCard" class="card">
            ...
            <div class="card card-sub-data ">
                <div class="card-body">
                    ...
                    <script>
                          var data = <?php echo json_encode($pat_breakdown_data); ?>;
                          if (data.length == 0) {
                            data = [100];
                          }
                          var label = <?php echo json_encode($pat_breakdown_labels); ?>;
                          if (label.length == 0) {
                            label = ['PAT no data'];
                          }
...
if(gplPercentage <= 80 || {{$gpl->ytd_revenue}} < 0){
...
if(patPercentage <= 80 || {{$gpl->ytd_revenue}} < 0){
...
if(ebitdaPercentage <= 80 || {{$gpl->ytd_revenue}} < 0){
