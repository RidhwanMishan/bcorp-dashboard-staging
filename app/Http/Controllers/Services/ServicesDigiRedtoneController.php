...
class ServicesDigiRedtoneController extends Controller {
    ...
    public function __invoke() {
        ...
        $GLOBALS['entity_name'] = "REDtone International Berhad"; //Can change depending on what company
        ...
        if (session('email')) {
            ...
            $pool_revenue[] = async(function(){
                $value_revenue = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
                INNER JOIN reporting.max_date_with_actual maxd ON 1=1
            ...
            //     $value_revenue_detailed_cumulative = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
            //     (SELECT sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Revenue' AND entity_name=rpt.entity_name) as act_amount,
            //     (SELECT sum(target_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Revenue' AND entity_name=rpt.entity_name) as trgt_amount
            // FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
            // WHERE entity_name = '".$GLOBALS['entity_name']."'
            ...
            //     $value_revenue_detailed_cumatively_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
            //     (SELECT sum(target_amount)-sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Revenue' AND entity_name=rpt.entity_name) as cumulative_variance
            // FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
            // WHERE entity_name = '".$GLOBALS['entity_name']."'
            // ORDER by fiscal_month
            //     ");                
            ...
            $pool_cos[] = async(function(){
                $value_cos = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
            ...
            //     $value_cos_detailed_cumulative = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
            //     (SELECT sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Cost of Sales' AND entity_name=rpt.entity_name) as act_amount,
            //     (SELECT sum(target_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Cost of Sales' AND entity_name=rpt.entity_name) as trgt_amount
            // FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
            // WHERE entity_name = '".$GLOBALS['entity_name']."'
            // ORDER BY fiscal_month
            //     ");
            ...
            //     $value_cos_detailed_cumatively_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
            //     (SELECT sum(target_amount)-sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Cost of Sales' AND entity_name=rpt.entity_name) as cumulative_variance
            //     FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
            //     WHERE entity_name = '".$GLOBALS['entity_name']."'
            //     ORDER BY fiscal_month         
            //     ");
            ...
            $pool_gpl[] = async(function(){
                $value_gpl = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
        ...
        //     $value_gpl_detailed_cumulative = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Gross profit/(loss)' AND entity_name=rpt.entity_name) as act_amount,
        //     (SELECT sum(target_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Gross profit/(loss)' AND entity_name=rpt.entity_name) as trgt_amount
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month
        //     ");
        ...
        //     $value_gpl_detailed_cumatively_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(target_amount)-sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Gross profit/(loss)' AND entity_name=rpt.entity_name) as cumulative_variance
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month
        //     ");
           ...
           // End GROSS PROFIT / LOSS - Detailed Data -  quarterly Variance
            ...
            $pool_opex[] = async(function(){
                $value_opex = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
        ...
        //     $value_opex_detailed_cumulative = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Operating expense' AND entity_name=rpt.entity_name) as act_amount,
        //     (SELECT sum(target_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Operating expense' AND entity_name=rpt.entity_name) as trgt_amount
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month
        //     ");
        ...
        //     $value_opex_detailed_cumatively_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(target_amount)-sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='Operating expense' AND entity_name=rpt.entity_name) as cumulative_variance
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month         
        //     ");
           ...
           // End OPEX - Detailed Data -  quarterly Variance
            ...
            $pool_ebitda[] = async(function(){
                $value_ebitda = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
                INNER JOIN reporting.max_date_with_actual maxd ON 1=1
                WHERE account_name='EBITDA'
                     AND entity_name = '".$GLOBALS['entity_name']."'
                GROUP BY entity_name
                ");                
                return $value_ebitda;
            ...
            $pool_pat[] = async(function(){
                $value_pat = DB::connection('redshift')->select("SELECT entity_name,
                sum(dashboard_amount) AS ytd_revenue,
                sum(CASE WHEN pnl.date <= maxd.max_data_date THEN target_amount ELSE 0 END) AS ytd_target,
                (ytd_target-ytd_revenue) as ytd_variance
                FROM reporting.pnl_monthly pnl
        ...
        //     $value_pat_detailed_cumulative = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='PAT' AND entity_name=rpt.entity_name) as act_amount,
        //     (SELECT sum(target_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='PAT' AND entity_name=rpt.entity_name) as trgt_amount
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month
        //     ");
        ...
        //     $value_pat_detailed_cumatively_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month,
        //     (SELECT sum(target_amount)-sum(dashboard_amount) FROM reporting.pnl_monthly pnl WHERE pnl.fiscal_month::int<=rpt.fiscal_month::int AND account_name='PAT' AND entity_name=rpt.entity_name) as cumulative_variance
        //  FROM (SELECT DISTINCT entity_name, month_year, fiscal_month FROM reporting.pnl_monthly) AS rpt
        //  WHERE entity_name = '".$GLOBALS['entity_name']."'
        //  ORDER BY fiscal_month         
        //     ");
           ...
           foreach($breakdown_array as $breakdown){
               ...
               // Start MONTHLY ACTUAL / TARGET BREAKDOWN 
                ...
                $pool_breakdown_monthly_variance[] = async(function(){
                    $value_breakdown_monthly_variance = DB::connection('redshift')->select("SELECT entity_name, month_year, fiscal_month, sum(target_amount)-sum(dashboard_amount) as monthly_variance   
                    FROM reporting.pnl_monthly
                    WHERE fiscal_year=".$GLOBALS['fiscalyear']."
                        AND parent_account_name = '".$GLOBALS['parent_account_name']."'
                        AND account_name = '".$GLOBALS['breakdown_account_name']."'
                        AND entity_name = '".$GLOBALS['entity_name']."'
                    GROUP BY entity_name, month_year, fiscal_month
                    ORDER BY fiscal_month
                    ");                
                ...
                //dd($breakdown_monthly_variance_result);
             ...
             ");                
                ...
                // dd($breakdown_cumatively_variance_result);
        ...
        //dd($revenue_detailed_monthly_data_actual);
            ...
            if($r_berjaya) {
                return view('services.digi_redtone', [
